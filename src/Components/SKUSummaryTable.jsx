...
//         Tagging_Qty: item.totalRoundOff,
...
// // summary  → the aggregated BOM lines (source of truth for SKU / qty)
// // bomLines → Raw_Material subform rows fetched from the saved CRM record
// // ─────────────────────────────────────────────────────────────────────────────

// function mergeBOMData(summary, bomLines = []) {
//     // Build a lookup keyed by Item_Code (= SKU)
...
//     const qty = parseFloat(item.totalRoundOff) || 0;
...
//  *
//  * Props:
//  *   summary        – array of BOM line items (SKU, Description, totalRoundOff, …)
//  *   dealData       – Zoho Deal record object (needs .Account_Name.id)
//  *   dealId         – Zoho Deal record ID string
//  *   users          – Zoho users response: { users: [{ id }] }
//  *   existingBOMId  – optional; skip the on-mount search if you already know the BOM id
//  */
// export default function SKUSummaryTable({ summary, dealData, dealId, users, existingBOMId = null }) {
//     // "loading" while we check CRM for a saved BOM on mount
...
//     // Start with plain initRows; will be replaced once CRM fetch completes
//     const [rows, setRows] = useState(() => summary.map(initRow));
//     // Keep a snapshot of the last-saved rows so Cancel can revert cleanly
...
//             // If caller already provided the BOM id, fetch that record directly.
//             // Otherwise search by dealId.
//             let bomRecord = null;
...
//             if (cancelled) return;
//             console.log({ bomRecord })

//             if (bomRecord) {
//                 // Hydrate rows with saved pricing from the BOM subform
...
//                 setStatus("saved");
//             } else {
//                 // No BOM yet — rows stay as plain initRows, status = idle
...
//         // Only run inside Zoho; in dev, skip straight to idle
//         if (typeof ZOHO !== "undefined" && ZOHO?.CRM) {
//             load();
//         } else {
//             setStatus("idle");
...
//         if (editing) {
//             setRows(savedRows); // revert to last saved snapshot
//         }
//         setEditing((v) => !v);
//     }
...
//                 // Skip if already has valid LWP price
//                 if (
//                     row.lwpPrice &&
//                     Number(row.lwpPrice) > 0
//                 ) {
//                     return row;
//                 }

//                 const inv =
//                     await fetchSKUFromInventory(
//                         row.SKU
//                     );
...
//             return; // block row update — keep last valid value
//         }

//         // Clear error for this cell when value is valid
//         setRowErrors((prev) => {
//             const next = { ...prev };
...
//             if (success) {
//                 setBomId(id);
//                 setSavedRows(rows); // update the revert snapshot
//                 setStatus("saved");
//                 setEditing(false);
//                 setTimeout(() => setStatus("saved"), 4000); // keep "Saved ✓" visible
//             } else {
//                 setStatus("error");
//             }
//         } catch (err) {
...
//     // ── Totals ────────────────────────────────────────────────────────────────
//     const totalQty = rows.reduce((s, r) => s + (parseFloat(r.totalRoundOff) || 0), 0);
...
//                             {["#", "SKU", "Description", "Qty", "LWP Price", "Discount %", "Dealer Price", "Anodizing %", "FOB Price", "Amount"].map((h, i) => (
//                                 <th
//                                     key={h}
...
//                                     {editing
//                                         ? <NumberInput value={item.discountPct} onChange={(v) => handleChange(i, "discountPct", v)} />
//                                         : <span className="text-orange-600">{item.discountPct}%</span>
//                                     }
//                                 </td>

//                                 {/* Dealer Price — computed */}
...
//                                 <td className={`px-3 py-2 text-right font-mono ${editing ? "bg-amber-50" : ""}`}>
//                                     {editing
//                                         ? <NumberInput value={item.anodizingPct} onChange={(v) => handleChange(i, "anodizingPct", v)} />
...
export default function SKUSummaryTable({ summary, dealData, dealId, users, existingBOMId = null }) {
    ...
    // ✅ Track whether the initial CRM load has completed
    const mountDone = useRef(false);
    // ✅ Track the pricing the user last saved (keyed by SKU) so summary
    //    updates can preserve it without depending on savedRows in the effect
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            if (bomRecord) {
                ...
                // ✅ Persist the saved pricing into the ref so the summary
                //    effect can read it without a stale closure
        ...
        return () => { cancelled = true; };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []); // ✅ run ONCE on mount — empty deps, not [summary]

    // ── When summary changes after mount → auto-save to BOM if bomId exists ──────
    useEffect(() => {
        // ✅ Skip first render — mount effect handles that
        ...
        // ✅ If BOM already exists → auto-save immediately, stay "saved"
        // ✅ If no BOM yet → mark idle so user can manually save
        if (bomId) {
            setStatus("saving");
            ...
            ZOHO.CRM.API.updateRecord({
                ...
                .then((result) => {
                    const ok = result?.data?.[0]?.status === "success";
                    if (ok) {
                        // ✅ Update pricing ref with latest saved data
                        savedPricingRef.current = Object.fromEntries(
                            fresh.map((r) => [r.SKU, {
                                lwpPrice: r.lwpPrice,
                                discountPct: r.discountPct,
            ...
            // No BOM yet — user must manually save
            setStatus("idle");
        }
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (success) {
                // ✅ Update the pricing ref so future summary changes
                //    preserve what was just saved
                savedPricingRef.current = Object.fromEntries(
                    rows.map((r) => [r.SKU, {
                        lwpPrice: r.lwpPrice,
                        discountPct: r.discountPct,
    ...
    return (
        <div className="mt-8 font-sans">
            ...
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                    {/* ✅ Show save button when idle (unsaved) even if NOT editing */}
